...
- **用途**：仅用于 AI 生成专属绘本
- **存储方式**：上传至后端 `uploads/temp` 临时目录，生成完成后立即删除；订单记录中仅保留照片临时 URL 字符串（用于生成重试和故障排查），不包含实际图片文件
- **保留时间**：图片文件不保留，生成成功或最终失败后立即删除；URL 字符串随订单记录长期保留

### 3. 生成结果图片
- **收集内容**：AI 生成的九宫格绘本图片 URL（由第三方 AI 服务返回）
- **用途**：供客户端下载并本地制作 PDF
- **存储方式**：宝贝绘本服务端不保存生成的图片文件，仅保存第三方返回的下载 URL；客户端下载后仅保存在本地 Documents 目录
- **保留时间**：服务端本地不保存图片文件，URL 随订单记录保留
...
- **存储方式**：打包在 App 本地资源中
- **保留时间**：随 App 版本保留
- **授权说明**：示例照片中的宝宝为开发者自有素材，已获得监护人授权，不会用于其他任何目的
